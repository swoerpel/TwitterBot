...
def main(argv):
    ...
    mask = GetRandomImage(mask_source).convert('L')
    mask = ImageOps.grayscale(mask)
    mask = ImageOps.autocontrast(mask, cutoff=0)
    mask = ImageOps.posterize(mask, 1)

    top_im = GetRandomImage(image_source).convert('RGB')
    bottom_im = GetRandomImage(image_source).convert('RGB')
    ...
    composite.save(composite_dest+composite_name + '.png')
    # mask.save(composite_dest+composite_name + '_MASK.png')
    # bottom_im.save(composite_dest+composite_name + 'B.png')


def GetRandomImage(path, name=''):
    if(name == ''):
